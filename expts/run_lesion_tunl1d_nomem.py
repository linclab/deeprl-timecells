...
from envs.tunl_1d import TunlEnv_nomem
import numpy as np
import torch
...
def rehydration_experiment(env, net, n_total_episodes, lesion_idx):
    ...
    nomem_perf = np.zeros(n_total_episodes, dtype=np.int8)
    nomem_perf_prime = np.zeros(n_total_episodes, dtype=np.int8)
    ...
    for i_episode in range(n_total_episodes):  # one episode = one sample
        ...
        nomem_perf[i_episode] = reward
        nomem_perf_prime[i_episode] = reward_prime
    ...
    del stim, first_action, first_action_prime, nonmatch_perc, nonmatch_perc_prime

    return np.mean(nomem_perf), np.mean(nomem_perf_prime), np.mean(np.asarray(kl_div))
...
if __name__ == '__main__':
    ...
    # Load_model_path: nomem_40_lstm_128_5e-05/seed_101_epi999999.pt, relative to training/tunl1d_og
    ...
    data_dir = f'/network/scratch/l/lindongy/timecell/figures/fig_2/tunl1d_nomem/seed_{seed}'
    ...
    env_title = 'Non-mnemonic TUNL 1D'
    net_title = 'LSTM'
    ...
    main_dir = f'/network/scratch/l/lindongy/timecell/figures/lesion/tunl1d_nomem'
    ...
    for i_lesion_type, lesion_type in enumerate(['random', 'ramp', 'time']):
        ...
        for i_num_lesion, num_lesion in enumerate(n_lesion):
            ...
            for i_shuffle in range(num_shuffle):
                ...
                env = TunlEnv_nomem(len_delay, seed=seed)
                if p is not None:
