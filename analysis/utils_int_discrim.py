...
def compare_correct_vs_incorrect(resp, stim_all, correct_trial, analysis="population", resp2=None):
    ...
    if analysis == "decoding":
        ...
        for train_idx, test_idx in kf.split(np.arange(np.shape(corr_resp)[0])):
            ...
            p_matrix_corr = time_cross_decode(corr_train, corr_test, title="", bin_size=340, plot=False)[0]
            p_matrix_incorr = time_cross_decode(corr_train, incorr_resp, title="", bin_size=340, plot=False)[0]
        ...
        max_stim_len = 40
        fig, ax = plt.subplots()
        ax.plot(np.arange(max_stim_len), np.arange(max_stim_len), '--', color='gray', alpha=0.7, label="Decoded = actual time")
        ...
        ax.plot(np.arange(max_stim_len), np.mean(decoded_t_incorr, axis=0), linewidth=3, label="Decode on incorrect trials")
        ...
        figpath = "ccn_figures/single_unit/stim1/"
        single_cell_temporal_tuning(stim, corr_resp, incorr_resp, figpath)
        ...
        corr_resp = np.concatenate((corr_resp, corr_resp2), axis=0)
        incorr_resp = np.concatenate((incorr_resp, incorr_resp2), axis=0)
        plot_sorted_in_same_order(corr_resp, incorr_resp, 'Correct trials', 'Incorrect trials', big_title="", len_delay=40, n_neurons=512)



def plot_training_performance(window_size=10000):
    n_total_epi = 150000
    accuracies = np.zeros((4, n_total_epi))
    parent_dir = os.getcwd() + "/data/ccn"
    i = 0
    for directory in os.listdir(parent_dir):
        if "lstm" in directory:
            ...
            data = np.load(file)
            accuracies[i,:] = bin_rewards(data["correct_trial"][:n_total_epi], window_size)
            i += 1
...
def time_decode_all_stim_len(stim_resp, stim, title="", bin_size=1000):
    """
    ...
    for i_len, stim_len in enumerate(all_stim_len):
        ...
        p_matrix = time_decode(resp, int(stim_len), 512, bin_size, "")[0]
...
def time_cross_decode(resp_corr, resp_incorr, title="", bin_size=340, plot=True):
    global figpath
...
def plot_training_accuracy(stim, action_hist, window_size=1000, base_episode=0):
...
def plot_performance(stim, action_hist, type="matrix"):
    accuracy = np.equal(action_hist==1, np.greater(stim[:,0], stim[:,1]))*1
...
def plot_time_cell_sorted_same_order(stim, stim1_resp, stim2_resp):
    stim_set = np.sort(np.unique(stim))
    ...
    for stim_len in stim_set:
        ...
        plot_sorted_in_same_order(stim1_activities, stim2_activities, 'Stim 1', 'Stim 2', big_title=str(stim_len)+"_steps", len_delay=stim_len, n_neurons=num_neurons)


def plot_time_cell(stim, resp_hx, label, figpath='data/analysis/population/'):
    stim_set = np.sort(np.unique(stim))
    ...
    for stim_len in stim_set:
        ...
        make_piechart(n_ramp_neurons, n_seq_neurons, n_neurons, figpath, label, filename='neuron_counts_'+str(stim_len))
        plot_sorted_averaged_resp(cell_nums_seq, sorted_matrix_seq, title=label+str(stim_len)+'_Sequence cells', remove_nan=True, filepath=figpath, filename='seq_'+str(stim_len))
        plot_sorted_averaged_resp(cell_nums_ramp, sorted_matrix_ramp, title=label+str(stim_len)+'_Ramping cells', remove_nan=True, filepath=figpath, filename='ramp_'+str(stim_len))
        plot_sorted_averaged_resp(cell_nums_all, sorted_matrix_all, title=label+str(stim_len)+'_All cells', remove_nan=True, filepath=figpath, filename='all_'+str(stim_len))



def single_cell_temporal_tuning(stim, stim1_resp, stim2_resp, figpath, compare_correct=False):
    stim_set = np.sort(np.unique(stim))
    stim_set.astype(int)
    num_stim = np.max(np.shape(stim_set))
    if not compare_correct:
        for unit in range(np.shape(stim1_resp)[-1]):
            ...
            for stim_idx in range(num_stim):
                ...
                recep_field[stim_idx,:stim_len,0] = np.mean(np.squeeze(stim1_resp[episode_idx_1, :stim_len, unit]), axis=0)
                recep_field[stim_idx,:stim_len,1] = np.mean(np.squeeze(stim2_resp[episode_idx_2, :stim_len, unit]), axis=0)
            ...
            ax1.set_aspect('auto')
            ax2.set_aspect('auto')
            plt.savefig(figpath + str(unit) + '.png')
...
def plot_postlesion_performance(accuracy, compare="lesion type"):
    ...
    if compare=="lesion type":
        for i_stim, len_stim in enumerate(stim_set):
            ...
            ax.legend(frameon=False)
            plt.savefig(figpath+'stim_'+str(len_stim)+'.png')
        ...
        for i_lesion_type, lesion_type in enumerate(lesions):
            ...
            ax.set_title("Lesion experiment")
            ax.legend(frameon=False)
            plt.savefig(figpath+lesion_type+'.png')


def retiming(stim, stim1_resp, stim2_resp):
    '''
    ...
    '''
    figpath = "data/analysis/single_cell/temporal_tuning_across_blocks/"
    stim_set = np.unique(stim)
    n_neuron = np.shape(stim1_resp)[-1]
    for i_neuron in range(n_neuron):
        for i_stim2, stim2 in enumerate(stim_set):
            ...
            plt.savefig(figpath+"Stim2_"+str(stim2)+"/Unit_"+str(i_neuron)+".png")
        print("Unit", str(i_neuron), "finished.")


def linear_readout(stim, stim1_resp, stim2_resp):
    '''
    ...
    '''
    figpath = "data/analysis/single_cell/readout/"
    stim_set = np.unique(stim)
    n_neuron = np.shape(stim1_resp)[-1]
    for i_neuron in range(n_neuron):
        ...
        plt.savefig(figpath+"Unit_"+str(i_neuron)+".png")
        plt.close()


def decoding(stim, resp, data_path, shuffle=False, load_data=True, n_fold=10):
    # NORMARLIZATION???
    if shuffle:
        data_path = data_path + '/decoding_shuffle.npz'
    else:
        data_path = data_path + '/decoding.npz'
    ...
    if not load_data:
        ...
        for i in range(n_target):
            ...
            for train_index, test_index in kf.split(resp): # for each fold
                ...
                i_split += 1
        np.savez_compressed(data_path, accuracies=accuracies)
    else:
        accuracies = np.load(data_path)
        accuracies = accuracies['accuracies']
    fig, ax = plt.subplots()
    ax.bar(np.arange(n_target), np.mean(accuracies,axis=1), width=0.6)
    ax.errorbar(np.arange(n_target), np.mean(accuracies,axis=1), yerr=np.std(accuracies,axis=1), fmt='None', ecolor='dimgray')
