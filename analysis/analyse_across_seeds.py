...
# find all .npy files that match the name f'mem_40_lstm_128_0.0001_seed_{seed}_epi199999.pt_data.npz
# and extract the seed number
seed_list = []
...
load= True
if load:
    t_test_arr = np.load(os.path.join(save_dir, f't_test_arr_{n_total_episodes}_{n_shuffle}_{percentile}.npy'), allow_pickle=True).item()
    t_test_pred_arr = np.load(os.path.join(save_dir, f't_test_pred_arr_{n_total_episodes}_{n_shuffle}_{percentile}.npy'), allow_pickle=True).item()
    info_arr = np.load(os.path.join(save_dir, f'info_arr_{n_total_episodes}_{n_shuffle}_{percentile}.npy'), allow_pickle=True).item()
...
for each_seed in seed_list:
    ...
    # Decode time
    print("Decode time...")
    t_test, t_test_pred = time_decode_lin_reg(delay_resp, len_delay, n_neurons, 1000, title='all_cells', save_dir=seed_save_dir, save=True)
    t_test_arr[each_seed] = t_test
    t_test_pred_arr[each_seed] = t_test_pred

    # Mutual Information
    print("Mutual Information...")
    info = joint_encoding_information_time_stimulus(delay_resp, stim, save_dir=seed_save_dir,title=f'{each_seed}_{n_total_episodes}_{n_shuffle}_{percentile}', logInfo=False, save=True)
    info_arr[each_seed] = info

# Save results
print("Save results..")
np.save(os.path.join(save_dir, f't_test_arr_{n_total_episodes}_{n_shuffle}_{percentile}.npy'), t_test_arr)
np.save(os.path.join(save_dir, f't_test_pred_arr_{n_total_episodes}_{n_shuffle}_{percentile}.npy'), t_test_pred_arr)
np.save(os.path.join(save_dir, f'info_arr_{n_total_episodes}_{n_shuffle}_{percentile}.npy'), info_arr)
...
'''
# Plot results
# Plot time decoding across seeds
print("Plot time decoding across seeds...")
t_test_arr = np.array(list(t_test_arr.values())).flatten()
t_test_pred_arr = np.array(list(t_test_pred_arr.values())).flatten()
...
ax.set_title(f'Time decoding pooled across {len(seed_list)} seeds')
ax.set_xlim((np.min(t_test_pred_arr),np.max(t_test_pred_arr)))
ax.set_ylim((np.min(t_test_pred_arr),np.max(t_test_pred_arr)))
fig.savefig(os.path.join(save_dir, f'time_decoding_pooled_{n_total_episodes}_{n_shuffle}_{percentile}.svg'), dpi=300)

# Plot mutual information across seeds
print("Plot mutual information across seeds...")
info_arr = np.vstack(info_arr.values())
n_total_neurons = np.shape(info_arr)[0]
breakpoint()
stats = cbook.boxplot_stats(info_arr, labels=['Loc x Time', r'$Loc x R_Time$', r'$Time x R_Loc$'], bootstrap=10000)
for i in range(len(stats)):
    stats[i]['whislo'] = np.min(info_arr[:,i], axis=0)
    stats[i]['whishi'] = np.max(info_arr[:,i], axis=0)
fig, axs = plt.subplots(1,1)
fig.suptitle('Mutual Information pooled across seeds')
...
plt.savefig(os.path.join(save_dir, f'joint_encoding_info_pooled_{n_total_episodes}_{n_shuffle}_{percentile}.svg'))
