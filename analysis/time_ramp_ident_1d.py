...
parser.add_argument("--behaviour_only", type=bool, help="whether the data only includes performance data")
parser.add_argument("--plot_performance", type=bool, help="if behaviour only, whether to plot the performance plot")
parser.add_argument("--normalize", type=bool, help="normalize each unit's response by its maximum and minimum")
parser.add_argument("--separate_trial_types", type=bool, help="identify ramp and sequence cells separately for left-stim and right-stim trials and then combine")
...
if separate_trial_types:
    ...
    np.savez_compressed(save_dir + 'ramp_ident_results_separate.npz',
    ...
    cell_nums_ramp = np.where(ramp_cell_bool)[0]
    np.savez_compressed(save_dir + 'ramp_ident_results_combined.npz',
...
if separate_trial_types:
    ...
    np.savez_compressed(save_dir + 'seq_ident_results_separate.npz',
    ...
    cell_nums_seq = np.where(seq_cell_bool)[0]
    np.savez_compressed(save_dir + 'seq_ident_results_combined.npz',
...
if separate_trial_types:
    ...
    np.savez_compressed(save_dir + 'trial_reliability_results_separate.npz',
    ...
    np.savez_compressed(save_dir+'trial_reliability_results_combined.npz',
...
if separate_trial_types:
    ...
    np.savez_compressed(save_dir+'temporal_info_results_separate.npz',
    ...
    np.savez_compressed(save_dir+'temporal_info_results_combined.npz',
